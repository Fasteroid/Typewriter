...
namespace Typewriter.CodeModel.Implementation
{
    public sealed class MethodImpl : Method
    {
        ...
        private MethodImpl(IMethodMetadata metadata, Item parent, Settings settings)
        {
            _metadata = metadata;
            Parent = parent;
            Settings = settings;
        ...
        public static IMethodCollection FromMetadata(IEnumerable<IMethodMetadata> metadata, Item parent, Settings settings)
        {
            return new MethodCollectionImpl(metadata.Select(m => new MethodImpl(m, parent, settings)));
        }
    }
}